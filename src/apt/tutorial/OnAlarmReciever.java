...
private static final int NOTIFY_ME_ID=1337;
	...
	public void onReceive(Context ctxt, Intent intent) {
		...
		if (useNotification) {
			...
			PendingIntent i = PendingIntent.getActivity(ctxt, 0, new Intent(ctxt, AlarmActivity.class),Intent.FLAG_ACTIVITY_NEW_TASK);
			
			Notification note=new Notification.Builder(ctxt)			
			.setContentTitle("LunchList")
			.setContentText("It's time for lunch Doa! Are't you hungry?")
			.setSmallIcon(R.drawable.stat_notify_chat)
			.setAutoCancel(true)
			.setContentIntent(i)
			.getNotification();
			
			//note.setLatestEventInfo(ctxt,  "LunchList", "It's time for lunch Doa! Are't you hungry?", i);
